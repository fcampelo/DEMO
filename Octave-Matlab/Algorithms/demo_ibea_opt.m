function [fopt, xopt] = demo_ibea_opt(f, xrange, options)
%DEMO_IBEAEPS_OPT: Multi-objective optimization using the IBEA with eps-indicator
...
%--------------------------------------------------------------------------%
function P = selection(P, O, options)
...
for ii = 1:(musofar - options.mu)
   F_ind = eps_fitness(Fnew, options.kappa);
   [~, imin] = min(F_ind);
